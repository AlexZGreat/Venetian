...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec3 color1 = 1-vec3(0.1,0.1,0.1);
    vec3 color2 = 1-vec3(0.2,0.2,0.2);
    
    float blend = time_but_works*0.03+noise(uv+screen_coords/40.0+time_but_works*0.10);
    
    blend = 0.5+0.5*sin(20.0*blend);

    vec3 color = mix(color1,color2,step(blend,0.5));
