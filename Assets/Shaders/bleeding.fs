...
extern PRECISION vec2 bleeding;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec3 color1 = 1-vec3(0.505,0.004,0.038);
    vec3 color2 = 1-vec3(0.645,0.210,0.002);
    
    float blend = time_but_works*0.03+noise(vec2(uv.x+screen_coords.x/40.0,uv.y+screen_coords.y/40.0+sin(uv.x*20.0+time_but_works*2.0)+time_but_works*0.10));
    
    blend = 0.5+0.5*sin(20.0*blend);

    vec3 color = mix(color1,color2,step(blend,0.1));
