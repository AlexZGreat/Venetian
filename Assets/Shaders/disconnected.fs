...
extern PRECISION vec2 disconnected;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec3 color1 = vec3(0.014,0.197,0.860);
    vec3 color2 = vec3(0.000,1.000,0.386);
    
    float blend = 0.5+(noise(uv*50.0+400.0*random2(vec2(time_but_works))));
    
    vec3 color = vec3(blend);
    
    float blend2 = random2(floor(vec2(uv.x*8.0,uv.y*3.0)+400.0*random2(vec2(time_but_works)))).x;
    
    vec3 color3 = mix(color2,color1,blend2);
    
    color = mix(color,color3,0.2);
